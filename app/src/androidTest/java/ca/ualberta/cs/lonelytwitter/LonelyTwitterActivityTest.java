...
public class LonelyTwitterActivityTest extends ActivityInstrumentationTestCase2 {
    ...
    public void testEditTweet() {
        ...
        Tweet newestTweet = oldTweetsList.getItemAtIndex(0);
        assertEquals(tweetText, newestTweet.getText());

        activity.runOnUiThread(new Runnable() {
            public void run() {
                oldTweetsList.getChildAt(0);
                oldTweetsList.performItemClick(v,0,getId());
            }
        });

        oldTweetsList.setOnItemClickListener(new AdapterView.OnItemLongClickListener()){
       ...
       newTweetsList.setOnItemClickListener(new AdapterView.OnItemLongClickListener()){
            public void onItemClick(AdapterView<?> parent, View view, int position, long id);
            Intent intent = new Intent(this, EditTweetActivity.class);
            startActivity(intent);

        activity.runOnUiThread(){
            view v = newTweetsList.performItemClick(v, 1, getId());
        };
        assertEquals("start with right tweet", EditTweetActivity.class, recieverActivity.getClass());
